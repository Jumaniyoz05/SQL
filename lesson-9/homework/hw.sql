...
select Departments. DepartmentName, Employees. Name  from Departments,Employees
...
select Orders.OrderID, Customers.CustomerID, Customers.FirstName from Orders,Customers
--5 Using Courses, Students table Get all combinations of students and courses.
select Customers.FirstName ,Students.Name from Customers cross join  Students
...
SELECT Customers.CustomerName, Sales.ProductID, Sales.Amount
FROM Sales
...
SELECT Orders.OrderID, Payments.Amount AS PaidAmount, Orders.TotalAmount
FROM Orders
JOIN Payments ON Orders.OrderID = Payments.OrderID
...
SELECT E1.EmployeeName AS Manager, E2.EmployeeName AS Employee, E1.Salary AS ManagerSalary, E2.Salary AS EmployeeSalary
FROM Employees E1
