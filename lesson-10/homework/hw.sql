...
--write a query to return the names and salaries of employees whose salary is greater than 50000, along with their department names.


select Name ,salary,Departments.DepartmentName  from Departments join Employees
...
select Employees.Name,Departments.DepartmentName from Employees left join Departments on Departments.DepartmentID=Employees.DepartmentID
...
select Suppliers.SupplierName, Products.ProductName from Products right join Suppliers on Products.SupplierID=Suppliers.SupplierID
...
select 
 ...
 m.name managername 
from 
employees as e left join Employees as m on m.employeeid=e.managerid

--Using the Customers and Orders tables, write a query to find customers who have placed an order with more than 3 items.
--Return their name and the quantity they ordered.🔁 Expected Columns: FirstName, LastName, Quantity
select * from orders
select * from customers
 select firstname,lastname ,quantity from  orders left join customers on orders.customerid=customers.customerid
 where quantity >3
...
select employees.name as employeename , DepartmentName from employees full join departments on employees.departmentid=departments.departmentid
...
select DepartmentName,  count(employeeid)as employeeCount from employees full join departments on employees.departmentid=departments.departmentid
...
select PRODUCTS.PRODUCTid,productNAME from products  left join sales on products.productid=sales.productid
where PRODUCTS.PRODUCTid is null

--12 Using the Customers and Orders tables, write a query to return customer names who have placed at least one order.
--🔁 Expected Columns: FirstName, LastName, TotalOrders
select * from orders
select FirstName, LastName, quantity as TotalOrders from customers full join orders on orders.customerid=customers.customerid
where quantity >=1
