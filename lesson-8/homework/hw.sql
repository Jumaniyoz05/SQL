...
select Category,sum (StockQuantity) as catagory_sum from Products
group by Category
--Using Products table, get the average price of products in the 'Electronics' category.
select  Category ,avg (Price) as avg_ca from Products
group by Category
...
select *, iif(price>30,'Low Stock','Sufficient') as filtee from  products 
--8 Using Customers table, find the total number of customers in each country.

select Country, count(firstname ) as count_fristname from Customers
group by Country
...
FROM Orders 
LEFT JOIN Invoices i ON OrderID = OrderID
WHERE OrderDate >= '2023-01-01' 
 ...
 select * from Products
  ...
  select YEAR(OrderDate)as year_,avg(TotalAmount)as avg_ from Orders
  ...
  SELECT 
    city_population table    [2012] AS Population_2012,
...
select ProductID,sum(SaleAmount) as sum_salemount from Sales
group by ProductID
...
--+1
--21Using Products and Sales tables, list product names and the number of times each has been sold. (Research for Joins)
SELECT 
    p.product_name,
