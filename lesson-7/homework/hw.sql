...
--SELECT Department,COUNT(EMPID) N_OF_EPMS FROM EmployeeSalaries
--GROUP BY Department
--HAVING COUNT(EMPID) > 1

--select DepartmentName , COUNT(EmployeeID) AS EMPLOYES_ from Employees
...
--SELECT ProductID,AVG (SaleAmount) AS AVG_ FROM Sales
...
select firstname, min (salary)as min_salary,
     firstname,  max (salary)as max_salary from employees 
	   group by FirstName
...
select DEPARTMENTNAME, AVG (SALARY)AS AVG_SALARY , COUNT(*) AS COUNT_NAME FROM employees 
GROUP BY DepartmentName
...
SELECT * FROM Orders
WHERE YEAR(ORDERDATE) = 2022 AND MONTH(OrderDate) = 10
