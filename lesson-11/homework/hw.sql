...
select orderid, Customers.FirstName,OrderDate from Orders join Customers on Orders.CustomerId=Customers.CustomerID
...
select Departments.DepartmentName,max(salary) as maxsalary from Employees join Departments on Employees.DepartmentID=Departments.DepartmentID
group by DepartmentName

--4 Return: CustomerName, OrderID, OrderDate
--Task: List all customers from the USA who placed orders in the year 2023.
--Tables Used: Customers, Orders
select Customers.FirstName,Orders.OrderID,Orders.OrderDate from Customers join Orders on Customers.CustomerID=Orders.CustomerID
where Country='usa'

--5--Return: CustomerName, TotalOrders
--Task: Show how many orders each customer has placed.
--Tables Used: Orders , Customers
select C.FirstName, count(quantity) as  TotalOrders  from Customers as c join Orders as o on c.CustomerID=o.CustomerID
...
select Customers.FirstName,max(orderdate) as  MostRecentOrderDate from Customers left join Orders on Customers.CustomerID=Orders.CustomerID
...
select Customers.FirstName,Orders.TotalAmount from Customers join Orders on Customers.CustomerID=Customers.CustomerID
where TotalAmount>500

--9 Return: ProductName, SaleDate, SaleAmount
--Task: List product sales where the sale was made in 2022 or the sale amount exceeded 400.
--Tables Used: Products, Sales
select Products.ProductName,Sales.SaleDate,Sales.SaleAmount from Products join Sales on Products.ProductID=Sales.ProductID
where SaleAmount>400 or SaleDate='2022'
--10
SELECT 
    p.ProductName, 
...
WHERE 
    c.Country = 'USA' AND c.Address REGEXP '^[0-9]{4}';
	--15
	SELECT 
    p.ProductName, 
    p.Category, 
...
WHERE 
    ...
    OR LENGTH(REGEXP_REPLACE(LOWER(e.Name), '[^aeiou]', '')) >= 4;
	--19
	SELECT 
    e.Name, 
    d.DepartmentName, 
