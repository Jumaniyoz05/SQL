...
SELECT FIRSTNAME AS CLIEND  FROM Customers
...
--INTERSECT. -BU KESISHMA BOLADI YANI IKALASIDA HAM BORINI OLADI 
SELECT * FROM  Products
INTERSECT
 SELECT * FROM Products_Discounted 

 --5. Write a query to select distinct customer names and their corrsponding Country using  -

 --  Distinct - farq qiluvchiМГГBK
...
select *,IIF (PRODUCTID > 20 ,'YES','NO') from  Products_Discounted
...
-- Find customer IDs who have placed orders but do not have a corresponding record in the Sales table
SELECT customerid
FROM Orders
EXCEPT
SELECT customerid
...
SELECT 
    ...
    quantity,
    CASE 
        WHEN quantity BETWEEN 1 AND 5 THEN 0
        WHEN quantity BETWEEN 6 AND 10 THEN 5
        WHEN quantity BETWEEN 11 AND 20 THEN 10
        WHEN quantity >= 21 THEN 15
        ELSE 0
