...
select concat(employee_id, ' '  ,  first_name ,'' , last_name  ) as  dd  from Employees
...
select phone_number stuff phone_number()
...
--SELECT phone_number, REPLACE(phone_number, '124', '999') AS updated_phone_number
--FROM employees
--WHERE phone_number LIKE '%124%';
...
--5. TestMax jadvalidagi har bir qator uchun yilni va Max1, Max2 va Max3 ustunlaridagi eng yuqori qiymatni olish uchun so'rov yozing.
...
--7. Siz ma'lumotlarni Id ga asosan tartibga solishingiz kerak, lekin Id qiymati 0 bo'lgan satrlar har doim oxirgi bo'lishi kerak. Endi savol shundaki,
--buni bitta tartib ustuni bilan bajara olasizmi? (BittaTartib)
select * from SingleOrder
order by (id = 0), id
 ...
 SELECT 
  ...
  CASE 
    WHEN LENGTH(FullName) - LENGTH(REPLACE(FullName, ' ', '')) = 2 THEN 
...
FROM Employees
WHERE (
  LENGTH(CONCAT(FirstName, LastName)) - LENGTH(REPLACE(LOWER(CONCAT(FirstName, LastName)), 'a', ''))
) >= 3;
SELECT 
  DepartmentID,
  COUNT(*) AS TotalEmployees,
  100.0 * SUM(CASE WHEN DATEDIFF(CURRENT_DATE, HireDate) > 365*3 THEN 1 ELSE 0 END) / COUNT(*) AS PercentOver3Years
FROM Employees
...
SELECT 
  JobDescription,
  MAX(ExperienceYears) AS MaxExperience,
  MIN(ExperienceYears) AS MinExperience,
  MAX(SpacemanID) KEEP (DENSE_RANK FIRST ORDER BY ExperienceYears DESC) AS MostExperiencedSpacemanID,
  MIN(SpacemanID) KEEP (DENSE_RANK FIRST ORDER BY ExperienceYears ASC) AS LeastExperiencedSpacemanID
FROM Personal
...
WITH ranked AS (
  SELECT
    ...
    SpacemanID,
    ExperienceYears,
    ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY ExperienceYears DESC) AS rn_desc,
    ROW_NUMBER() OVER (PARTITION BY JobDescription ORDER BY ExperienceYears ASC) AS rn_asc
  FROM Personal
)
SELECT 
  JobDescription,
  MAX(CASE WHEN rn_desc = 1 THEN ExperienceYears END) AS MaxExperience,
  MAX(CASE WHEN rn_asc = 1 THEN ExperienceYears END) AS MinExperience,
  MAX(CASE WHEN rn_desc = 1 THEN SpacemanID END) AS MostExperiencedSpacemanID,
  MAX(CASE WHEN rn_asc = 1 THEN SpacemanID END) AS LeastExperiencedSpacemanID
FROM ranked
GROUP BY JobDescription;
SELECT
  GROUP_CONCAT(SUBSTRING(chars, n, 1) ORDER BY n SEPARATOR '') AS OriginalString,
  GROUP_CONCAT(CASE WHEN SUBSTRING(chars, n, 1) REGEXP '[A-Z]' THEN SUBSTRING(chars, n, 1) ELSE NULL END ORDER BY n SEPARATOR '') AS Uppercase,
  GROUP_CONCAT(CASE WHEN SUBSTRING(chars, n, 1) REGEXP '[a-z]' THEN SUBSTRING(chars, n, 1) ELSE NULL END ORDER BY n SEPARATOR '') AS Lowercase,
  GROUP_CONCAT(CASE WHEN SUBSTRING(chars, n, 1) REGEXP '[0-9]' THEN SUBSTRING(chars, n, 1) ELSE NULL END ORDER BY n SEPARATOR '') AS Numbers,
  GROUP_CONCAT(CASE WHEN SUBSTRING(chars, n, 1) NOT REGEXP '[A-Za-z0-9]' THEN SUBSTRING(chars, n, 1) ELSE NULL END ORDER BY n SEPARATOR '') AS Others
FROM
  (SELECT 'tf56sd#%OqH' AS chars) AS t
JOIN
  (SELECT 1 AS n UNION ALL SELECT 2 UNION ALL SELECT 3 UNION ALL SELECT 4 UNION ALL SELECT 5 UNION ALL SELECT 6 UNION ALL SELECT 7 UNION ALL SELECT 8 UNION ALL SELECT 9 UNION ALL SELECT 10) nums
ON n <= LENGTH(chars);
SELECT 
  ID, 
  Value,
  SUM(Value) OVER (ORDER BY ID ROWS BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW) AS RunningTotal
FROM Students
ORDER BY ID;
SELECT BirthDate, GROUP_CONCAT(StudentID) AS StudentsWithSameBirthday, COUNT(*) AS Count
FROM Students
GROUP BY BirthDate
HAVING COUNT(*) > 1;
SELECT
